  ...
  return (
    <a href="#home" className="flex items-center gap-3">
      ...
      <span className="leading-none">
        <span className={`block font-display text-lg tracking-[0.14em] ${dark ? 'text-cream-50' : 'text-maroon-800'}`}>VASTU ASTRO</span>
    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-cream-50/90 shadow-[0_8px_30px_-15px_rgba(74,15,30,0.35)] backdrop-blur-md' : 'bg-transparent'}`}>
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3.5 lg:px-8">
        ...
        <ul className="hidden items-center gap-8 lg:flex">
          {links.map((l) => (
            <li key={l.label}>
              <a href={l.href} className="relative text-sm font-medium text-maroon-900/75 transition-colors hover:text-maroon-800 after:absolute after:-bottom-1.5 after:left-0 after:h-px after:w-0 after:bg-gold-500 after:transition-all after:duration-300 hover:after:w-full">
                {l.label}
              </a>
            </li>
          ))}
      ...
      <div className={`overflow-hidden bg-cream-50/98 backdrop-blur-md transition-all duration-300 lg:hidden ${open ? 'max-h-96 border-t border-maroon-700/10' : 'max-h-0'}`}>
        <ul className="space-y-1 px-5 py-4">
          {links.map((l) => (
            <li key={l.label}>
              <a href={l.href} onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2.5 text-sm font-medium text-maroon-900/80 hover:bg-maroon-50">
                {l.label}
              </a>
            </li>
          ))}
